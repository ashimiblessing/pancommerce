...
//total cart sum in currency
const cartTotalSum = (cart) => {
 
  
  //sum the total items in currency
  const currencyTotal = Object.keys(cart).reduce((sum,key)=>sum+cart[key].quantity*cart[key].price||0,0) 
  setCartSum(parseFloat(currencyTotal).toFixed(2))
...
//define a function to refresh cart first when currency is changed

const refreshCart = (data) => {
  
  //don't refresh if cart is empty
  if(!cartContents){
    return
  }
 ...
 for(const child in data.products){
   const productId = data.products[child].id
   if(productId in cartCopy){
    
    const cartItem =  cartCopy[productId]
    cartItem.price = data.products[child].price
    cartCopy[productId] = cartItem
   }
 }
...
cartTotalSum(cartCopy)
  ...
  const {   loading, error, data, refetch } =  useQuery(DEFAULT_QUERY, {
    variables: { currency:itemCurrency },
    fetchPolicy: "cache-and-network",
    onCompleted: (data) => refreshCart(data)
  });
...
//total items for cart count
 const cartTotalItems = (cart) => {
 
 // calculates the current cart via reduction
  const total = Object.keys(cart).reduce((sum,key)=>sum+cart[key].quantity||0,0) 
...
const removeItem = (id) => {
 const cartCopy = {...cartContents}
 const filteredCopy =  Object.entries(cartCopy).filter(([key, value]) => value.id !== id) 
...
<select className="filter-select" value="new-products" onChange={() => filterProducts()}><option value="" disabled="">Filter by</option><option value="all-products">All Products</option><option value="new-products">New Products</option><option value="sets">Sets</option><option value="skincare">Skincare</option><option value="hair-and-body">Hair &amp; Body Care</option><option value="accessories">Accessories</option></select>
</div>
...
<div className="item-price">US${parseFloat(content.quantity * content.price).toFixed(2)}</div>
...
<span class="remove-product" onClick={() => removeItem(content.id)}>x</span>
  ...
  <p className="no-items" style={{display:`${cartContents.length < 1 ? 'block' :'none'}`}}>There are no items in your cart.</p>
    ...
    <div className="cart-subtotal"><span>Subtotal</span><div className="subtotal-price">{itemCurrency}&nbsp;{cartSum}</div></div>
